...
Image::Image() {}
Image::Image(std::string const &file_name, int flags) : fname_{file_name} {
  image_ = cv::imread(fname_);
  if (!image_.data) {
    std::cerr << "ERROR::Image() " << "Nao foi possivel abrir: " << fname_
              << '\n';
  }
}

auto Image::write_image(std::string const &out_file,
                        std::vector<int> const &params) -> bool {
...
auto Image::image() -> cv::Mat { return image_; }
auto Image::clone_from_rect(const cv::Rect &rect, std::string const &out_fname,
                            Image &out_image) -> void {
  out_image.image_ = image_(rect).clone();
  cv::rectangle(image_, rect, cv::Scalar(0, 255, 0), 2);
  write_image(out_fname);
}
